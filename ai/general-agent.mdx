...
# Filter for urgent assignments
def is_urgent(assignment):
    if not assignment.get('due_at'):
        return False
    due_date = datetime.fromisoformat(assignment['due_at'].replace('Z', '+00:00'))
    days_until_due = (due_date - datetime.now()).days
    return days_until_due <= 3 and days_until_due >= 0

urgent_assignments = [a for a in assignments if is_urgent(a)]

# Create todos for urgent assignments
